...
class AttrPicker extends React.Component<IProps> {
    public render() {
        ...
        if (!sAttrs.hasOwnProperty("properties")) throw new Error("JSON missing the 'properties' attribute.");
        return (
            <div id="attrPicker" style={{display: "flex", flexWrap: "wrap", flexDirection: "row"}}>
                {Object.keys(sAttrs.properties).map((ppty: string) => (
                    <LabeledInput key={`inp-${ppty}`} modAttr={modAttr} inputProps={sAttrs.properties[ppty]} eAttr={ppty} eValue={(shape.properties[ppty])}/>
                ))}
            </div>
        )
    }
