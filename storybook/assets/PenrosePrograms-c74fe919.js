import{m as e}from"./index-441ec744.js";const t="type Object",o=`canvas {
    width = 400
    height = 400
}
...
Circle C
Ellipse E
`,f={"closest-point.domain":p,"closest-point.style":g,"test.substance":u},h=`type Point
type Ellipse
type VectorSpace
type Vector
...
Ellipse e1
Ellipse e2
`,k={"closest-point-ellipse.domain":h,"closest-point-ellipse.style":b,"closest-point-ellipse.substance":v},x=`canvas {
    width = 600
    height = 400
}
...
`,P={"closed-elastic-curve.style":x,"cubic-bezier.style":m,"cubic-bezier.substance":y,"curves.domain":w,"homotopy.style":C,"homotopy.substance":L,"open-elastic-curve.style":A},S=`type Graph
type Variable
...
}`,O={"existential-graph.domain":S,"graph.substance":E,"style.style":T},B=`type Scalar
...
`,M=`-- define the size of the drawing
canvas {
   width = 240
   height = 180
}
...
Label u $a \\wedge b$
Label v $b \\wedge c$
`,V={"exterior-algebra.domain":B,"exterior-algebra.style":M,"vector-wedge.substance":G},I=`type AmbientSpace
type Hyperplane
type Vector
...
AutoLabel All
`,Q={"fake3d.domain":I,"fake3d.style":N,"projection.substance":_},q=`type Text
type Point
`,D=`canvas {
   scalar width  = 533.
   scalar height = 300.
...
`,j={"fancy-text.domain":q,"fancy-text.style":D,"fancy-text.substance":R},W=`type Frame
type Moon
type Crater
type Star
type Branch
...
`,H={"full-moon.domain":W,"full-moon.style":$,"full-moon.substance":F},z=`-- Original Style: https://github.com/penrose/penrose/blob/90e88c5a075d6a75698c49c2feb101275ae64af8/examples/geometry-domain/euclidean.sty
...
-- notation "∠(p, q, r)" ~ "InteriorAngle(p, q, r)"`,Y=`-- First construct the right triangle
-- Byrne (1)
Point A, B, C
Triangle ABC := {A, B, C} 
Angle CAB := ∠(C, A, B)
...
AutoLabel All
`,Z=`-- First construct the right triangle
Point A, B, C
Triangle ABC
ABC := MkTriangle(A, B, C) 
...
AutoLabel p, q, r, s, m
Label P $E^2$`,nn=`-- substance name in registry: collinear
Plane R
Point A, B, C, D, E
In(A, R)
In(B, R)
...
Line BA := MkLine(B, A)
Collinear(B,D,E)
AutoLabel A, B, C, D, E, R`,en=`Point M, N, Q, P
...
EqualLength(MN, QP)
AutoLabel M, N, Q, P`,tn=`Point A, B, C, D
Ray DA := MkRay(D, A)
Ray DB := MkRay(D, B)
Ray DC := MkRay(D, C)
...
AutoLabel A, B, C, D`,on=`-- name in registry: parallel-lines
Point J, K, L, M, N
Line JK := MkLine(J, K)
Line NL := MkLine(N, L)
Line KM := MkLine(K, M)
...
-- Acute(JKM) 
AutoLabel J, K, L, M, N`,rn=`Point A, B, C, D, E
...
AutoLabel A, B, C, D, E
Label aCDE $x$
`,ln=`-- name in registry: midsegment-triangles
Point D, E, F, G, H, J, K
...
AutoLabel D, E, F, G, H, J, K`,sn=`-- name in registry: incenter-triangle
Point J, K, L, P, m
...
AutoLabel J, K, L, M, N`,dn=`Point A, B, C, X, Y, Z, P
Triangle ABC := MkTriangle(A, B, C)
Segment AB := MkSegment(A, B)
Segment BC := MkSegment(B, C)
Segment AC := MkSegment(A, C)
...
AutoLabel A, B, C, X, Y, Z, P`,pn=`Point A, B, C, D, E
Line AB := MkLine(A, B)
Line CD := MkLine(C, D)
Parallel(AB, CD)
...
EqualLength(AB, CD)
AutoLabel A, B, C, D, E`,gn=`Point F, G, H, J, K, L
...
EqualAngle(GFL, LFH)
AutoLabel F, G, H, J, K, L`,un=`Point R, S, T, U, V, W
...
EqualLength(UV, VW)
AutoLabel R, S, T, U, V, W`,fn=`Point P, Q, R, S, T
...
AutoLabel P, Q, R, S, T`,hn=`Point A, B, C, D
Triangle ABC := MkTriangle(A, B, C)
Segment BD := MkSegment(B, D)
Collinear(A, D, C)
AutoLabel All`,bn=`Point J, K, L
Triangle JKL := MkTriangle(J, K, L)
Segment KL := MkSegment(K, L)
Segment j := PerpendicularBisector(KL, J)
AutoLabel All`,vn=`-- TODO one of the triangles crosses with the other segments on a regular basis
Point A, B, C, D, E, F
Triangle ABC := MkTriangle(A, B, C)
...
AutoLabel A, B, C, D, E, F`,kn=`-- name in registry: circle-example
Point P, Q, U, S, T, R
Circle c
Segment PR := MkSegment(P, R)
...
AutoLabel P, Q, U, S, T, R`,xn=`-- TODO full example can sometimes throw error "2147482504"
...
AutoLabel A, B, C, D, E, F, d, b`,mn=`Point J, K, L, M, Y
Circle c
Segment LM := Chord(c, L, M)
Segment JK := Chord(c, J, K)
Collinear(J, Y, K)
Collinear(L, Y, M)
AutoLabel All`,yn=`Point A, B, C, D, O
Circle c
CircleCenter(c, O)
...
Parallel(AB, DC)
AutoLabel A, B, C, D, O`,wn=`Point T, V, S, P
...
AutoLabel All`,Cn=`Point A, B, C, D
Triangle ABC := MkTriangle(A, B, C)
...
Parallel(AC, BD)
EqualLength(AC, BD)
AutoLabel A, B, C, D`,Ln=`-- TODO simplify after attributes are implemented
...
AutoLabel U, S, T, X, Y, Z`,An={"c01p01.substance":nn,"c01p10.substance":en,"c02p01.substance":tn,"c03p01.substance":on,"c04p01.substance":rn,"c04p12.substance":an,"c05p01.substance":ln,"c05p13.substance":sn,"c06p06.substance":cn,"c07p02.substance":dn,"c07p06.substance":pn,"c07p10.substance":gn,"c07p22.substance":un,"c08p08.substance":fn,"c08p18.substance":hn,"c09p02.substance":bn,"c10p08.substance":vn,"c11p07.substance":kn,"c11p12.substance":xn,"c11p21.substance":mn,"c11p25.substance":yn,"c12p12.substance":wn,"c12p20.substance":Cn,"ex.substance":Ln},Pn={"euclidean-teaser.style":z,"euclidean.style":U,"general-triangle.substance":K,"geometry.domain":X,"pythagorean-theorem-sugared.substance":Y,"pythagorean-theorem-unsugared.substance":Z,"teaser.substance":J,textbook_problems:An},Sn=`type Vertex
type Link
...
AutoLabel All
`,Vn={"arpanet.substance":Tn,"evolutionary-tree.substance":On,"glycolysis-pathway.substance":Bn,"krebs-cycle.substance":Mn,"nyc-subway.substance":Gn},In=`type Vertex
type Link
...
Label e "𝑒"
Label f "𝑓"
`,Yn=`Vertex Chicago, Denver, Detroit, LA, NYC, SF, Washington
...
Label h "RRP46"
Label i "PM/Sci2"
`,ee=`Vertex team1, team2, team3, team4, team5, team6
...
Label team5 "Team 5"
Label team6 "Team 6"
`,te=`Vertex Chicago, Denver, Detroit, LA, NYC, SF, Washington
...
Label NYC "New York"
Label SF "San Francisco"
`,oe=`Vertex Chicago, Denver, Detroit, LA, NYC, SF, Washington
...
Label NYC "New York"
Label SF "San Francisco"
`,re=`Vertex Chicago, Denver, Detroit, LA, NYC, SF, Washington
...
Label NYC "New York"
Label SF "San Francisco"
`,ae=`Vertex Chicago, Denver, Detroit, LA, NYC, SF, Washington
...
AutoLabel All
`,pe={"ex3.substance":$n,"ex4.substance":Fn,"ex5.substance":Hn,"ex6.substance":zn,"ex7.substance":Un,"ex8.substance":Kn,"ex9.substance":Xn,"fig1.substance":Yn,"fig10.substance":Zn,"fig11.substance":Jn,"fig12.substance":ne,"fig13.substance":ee,"fig2.substance":te,"fig3.substance":oe,"fig4.substance":re,"fig5.substance":ae,"fig6.substance":ie,"fig7.substance":le,"fig8a.substance":se,"fig8b.substance":ce,"fig9.substance":de},ge=`Vertex a, b, c, d, e, f
...
Label d "𝑑"
Label e "𝑒"
`,Ae=`Vertex Alvarez, Berkowitz, Chen, Davis
...
AutoLabel All
`,Pe=`Vertex Washington, Xuan, Ybarra, Ziegler
Vertex requirements, architecture, implementation, testing
...
Label p32 "𝑃(3, 2)"
Label p33 "𝑃(3, 3)"
`,Me=`Vertex p0, p1, p2, p3, p4, p5, p6, p7
...
Edge(k26t2, k26b5)
Edge(k26t2, k26b6)
`,$e={"eg9.substance":ge,"ex1.substance":ue,"ex2.substance":fe,"ex21.substance":he,"ex22.substance":be,"ex23.substance":ve,"ex24.substance":ke,"ex25.substance":xe,"ex3.substance":me,"ex51.substance":ye,"ex7.substance":we,"ex8.substance":Ce,"ex9.substance":Le,"fig10a.substance":Ae,"fig10b.substance":Pe,"fig11a.substance":Se,"fig11b.substance":Ee,"fig11c.substance":Te,"fig12.substance":Oe,"fig13.substance":Be,"fig14.substance":Me,"fig16b.substance":Ge,"fig1G.substance":Ve,"fig1H.substance":Ie,"fig2.substance":Ne,"fig3.substance":_e,"fig4.substance":Qe,"fig5.substance":qe,"fig6.substance":De,"fig8G.substance":Re,"fig8H.substance":je,"fig9.substance":We},Fe=`Vertex a, b, c, d
...
Label v4 "𝑣₄"
Label v5 "𝑣₅"
`,ot=`Vertex u1, u2, u3, u4, u5, u6, u7
...
Label v5 "𝑣₅"
Label v6 "𝑣₆"
`,st=`Vertex u1, u2, u3, u4, u5, u6, u7, u8
...
Label v7 "𝑣₇"
Label v8 "𝑣₈"
`,ct=`Vertex u1, u2, u3, u4, u5, u6, u7, u8, u9, u10
...
Label v9 "𝑣₉"
Label v10 "𝑣₁₀"
`,dt=`Vertex u1, u2, u3, u4, u5, u6, u7, u8, u9, u10
...
Label v9 "𝑣₉"
Label v10 "𝑣₁₀"
`,pt=`Vertex u1, u2, u3, u4, u5, u6, u7, u8
...
Label v3 "𝑣₃"
Label v4 "𝑣₄"
`,Pt={"ex1.substance":Fe,"ex13.substance":He,"ex14.substance":ze,"ex15.substance":Ue,"ex19.substance":Ke,"ex2.substance":Xe,"ex20.substance":Ye,"ex21.substance":Ze,"ex3.substance":Je,"ex34.substance":nt,"ex35.substance":et,"ex36.substance":tt,"ex37.substance":ot,"ex38.substance":rt,"ex39.substance":at,"ex4.substance":it,"ex40.substance":lt,"ex41.substance":st,"ex42.substance":ct,"ex43.substance":dt,"ex44.substance":pt,"ex50.substance":gt,"ex61.substance":ut,"ex62.substance":ft,"ex63.substance":ht,"ex64.substance":bt,"fig1.substance":vt,"fig10.substance":kt,"fig11.substance":xt,"fig12.substance":mt,"fig2.substance":yt,"fig3.substance":wt,"fig4.substance":Ct,"fig5.substance":Lt,"fig8.substance":At},St=`Vertex a, b, c, d, e
...
Label f "𝑓"
Label g "𝑔"
`,Gt={"ex11a.substance":St,"ex11b.substance":Et,"ex11c.substance":Tt,"ex12a.substance":Ot,"ex12b.substance":Bt,"ex12c.substance":Mt},Vt=`Vertex a, b, c, d
...
Label f "𝑓"
Label g "𝑔"
`,Dt=`Vertex a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q
...
Edge(d, f)
Edge(e, f)
`,Ut={"ex18.substance":Vt,"ex21.substance":It,"ex30.substance":Nt,"ex31.substance":_t,"ex32.substance":Qt,"ex33.substance":qt,"ex34.substance":Dt,"ex35.substance":Rt,"ex36.substance":jt,"ex46.substance":Wt,"ex47a.substance":$t,"ex47b.substance":Ft,"ex47c.substance":Ht,"ex47d.substance":zt},Kt=`Vertex a, b, c, d, e, f
...
Edge(h, g)
Edge(e, g)
`,Yt={"ex5.substance":Kt,"fig4.substance":Xt},Zt={sec1:pe,sec2:$e,sec3:Pt,sec4:Gt,sec5:Ut,sec7:Yt},Jt={"directed-multigraph.domain":Sn,"directed-multigraph.style":En,"other-examples":Vn,"pseudograph.domain":In,"pseudograph.style":Nn,"simple-curved-graph-boxes.style":_n,"simple-curved-graph-dots.style":Qn,"simple-curved-graph.style":qn,"simple-directed-graph.domain":Dn,"simple-directed-graph.style":Rn,"simple-graph.domain":jn,"simple-graph.style":Wn,textbook:Zt},no=`-- Draws a Cayley graph for a group G, assuming that
...
`,eo=`type Element -- an element of the group
...
Label g9 $g_{9}$
Label g10 $g_{10}$
`,ao=`-- variation: LeverkaasAlbatross41817
Element g1
Element g2
Element g3
Element g4
...
Label g7 $-j$
Label g8 $-k$
`,io={"dihedral-1.substance":oo,"dihedral-5.substance":ro,"quaternions.substance":ao},lo={"CayleyGraph.style":no,"Group.domain":eo,"MultiplicationTable.style":to,groups:io},so={},co=`canvas {
   scalar width  = 533.
   scalar height = 300.
}
...
`,fo={Notes:so,"PoincareDisk.style":co,"hyperbolic-example-unsugared.substance":po,"hyperbolic-example.substance":go,"hyperbolic.domain":uo},ho=`<svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 700">
  <rect
    x="291.702230696695"
    y="345.9981178395118"
    width="20"
...
`,mo={"example.svg":ho,"hypergraph-3d.style":bo,"hypergraph.domain":vo,"hypergraph.style":ko,"hypergraph.substance":xo},yo=`Node a, b, c, d
Node u, v, x, y, z
...
`,wo=`<svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 240 180">
  <rect
    x="0"
    y="0"
    width="240"
...
`,Ao={"example.substance":yo,"example.svg":wo,"lagrange-bases.domain":Co,"lagrange-bases.style":Lo},Po=`VectorSpace V
Vector u 
Vector v 
In(u, V)
In(v, V)
...
Orthogonal(x1, x2)
AutoLabel All
`,Io={"dependent.substance":Po,"error-test.style":So,"independent.substance":Eo,"linear-algebra-paper-simple.style":To,"linear-algebra-test.domain":Oo,"linear-algebra.domain":Bo,"linearalgebra-paper-dashes.style":Mo,"twoVectorsPerp-unsugared.substance":Go,"twoVectorsPerp.substance":Vo},No=`canvas {
    width = 800
    height = 700
}
...
Connection C16 := MakeConnection(XOR3OUT, CARRY)`,Qo=`AutoLabel All
...
Connection C8 := MakeConnection(ANDOUT, CAR)`,qo=`-- Nodes
...
constructor MakeConnection(Node A, Node B) -> Connection`,Do={"distinctive-shape.style":No,"full-adder.substance":_o,"half-adder.substance":Qo,"logic-gates.domain":qo},Ro=`type Scalar
type Vector
type Matrix
...
`,ar={"matrix-matrix-addition.substance":Wo,"matrix-matrix-division-elementwise.substance":$o,"matrix-matrix-multiplication-elementwise.substance":Fo,"matrix-matrix-multiplication.substance":Ho,"matrix-matrix-subtraction.substance":zo,"matrix-transpose.substance":Uo,"matrix-vector-left-multiplication.substance":Ko,"matrix-vector-right-multiplication.substance":Xo,"scalar-vector-division.substance":Yo,"scalar-vector-left-multiplication.substance":Zo,"scalar-vector-right-multiplication.substance":Jo,"vector-vector-addition.substance":nr,"vector-vector-division-elementwise.substance":er,"vector-vector-multiplication-elementwise.substance":tr,"vector-vector-outerproduct.substance":or,"vector-vector-subtraction.substance":rr},ir={"matrix-ops.domain":Ro,"matrix-ops.style":jo,tests:ar},lr=`-- Clashing notations with \\in symbol
...
predicate Identified(Type, Type)`,cr=`-- Random seeds; first seed controls the vertex selection, second seed controls the mesh topology
...
}`,dr=`-- Clashing notations with \\in symbol
-- Vertex v1, v2, v3, v4 -- ∈ K (can't say this)
...
-- Also identify faces with sets
Identified(f, A)
`,pr={"DomainInterop-unsugared.substance":lr,"DomainInterop.domain":sr,"DomainInterop.style":cr,"DomainInterop.substance":dr},gr=`--- This .style file is intended for debugging purposes only
...
`,ur=`--- This .style file is intended for debugging purposes only
...
Maze M
Triangle T1, T2, T3
`,vr={"ellipse-maze.style":gr,"ellipses-maze.style":ur,"maze.domain":fr,"maze.style":hr,"maze.substance":br},kr=`
type Shape
`,xr=`
canvas {
  width = 800
...
`,mr=`
Shape Yin, Yang
`,yr={"yin-yang.domain":kr,"yin-yang.style":xr,"yin-yang.substance":mr},wr={maze:vr,"yin-yang":yr},Cr=`type Center
type Shape
...
constructor Inversion( Shape original, Center center ) -> Shape`,Lr=`canvas {
    width = 800
    height = 700
}
...
Label ic1 $c'_1$
Label ic2 $c'_2$
Label ic3 $c'_3$`,Pr={"mobius.domain":Cr,"mobius.style":Lr,"mobius.substance":Ar},Sr=`type Object
type Morphism
...
constructor join(Object first, Object second) -> Morphism`,Er=`Colors {
...
-- AutoLabel All`,Or={"monoidal-test.domain":Sr,"monoidal-test.style":Er,"test.substance":Tr},Br=`<!-- <svg width="100%" height="100%" version="1.2" viewBox="0 0 240 180" xmlns="http://www.w3.org/2000/svg"> -->
...
`,Vr=`-- define the size of the drawing
canvas {
   width = 240
   height = 180
}
...
`,Ir={"penrose-sound-example.svg":Br,"sound-test.substance":Mr,"sound.domain":Gr,"sound.style":Vr},Nr=`<svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 700">
  <circle
    fill="#339933"
    fill-opacity="0.3"
...
`,jr={"example.svg":Nr,"persistent-homology-basic.style":_r,"persistent-homology-shading.svg":Qr,"persistent-homology.domain":qr,"persistent-homology.style":Dr,"persistent-homology.substance":Rr},Wr=`canvas {
  width = 800
  height = 700
}
...
AutoLabel All
`,Ur=`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 874.58 820.84">
  <defs>
...
predicate Intersecting(Set s1, Set s2)
predicate IsSubset(Set s1, Set s2)
`,Xr=`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 829.57 829.57">
  <defs>
...
`,e0=`-- Fork of venn-opt-test to verify doing optimization through an (external) computation
...
`,i0={"continuousmap.style":Wr,"continuousmap.substance":$r,"functions.domain":Fr,"multisets.substance":Hr,"nested.substance":zr,"set-theory-domain-shadow.svg":Ur,"setTheory.domain":Kr,"shading.svg":Xr,"tree.style":Yr,"tree.substance":Zr,"twosets-simple.substance":Jr,"venn-3d.style":n0,"venn-comp-test.style":e0,"venn-polygon.style":t0,"venn-small.style":o0,"venn-test.style":r0,"venn.style":a0},l0=`canvas {
  width = 800
  height = 700
}
...
}`,s0=`Point p1, p2, p3, p4, p5, p6, p7, p8
Polygon g
...
e2 := MkEdge(b1, b2)
e3 := MkEdge(b1, b2)
e4 := MkEdge(b1, b2)`,m0={"box-arrow.style":l0,"eight-points-around-polygon.substance":s0,"eighty-text.substance":c0,"ellipse.substance":d0,"hundred-points-around-line.substance":p0,"hundred-points-around-star.substance":g0,"lines-around-rect.substance":u0,"ninety-points-around-polyline.substance":f0,"rect-line-dist.style":h0,"shape-distance.style":b0,"shapes.domain":v0,"supersimplegraph.domain":k0,"two-nodes-two-edges.substance":x0},y0=`type Shape
type Circle <: Shape
type Ellipse <: Shape
...
`,P0={"all-shapes.domain":y0,"all-shapes.substance":w0,"arrows.style":C0,"arrows.substance":L0,"shape-spec.style":A0},S0=`type Node
`,E0=`-- Visualize the construction of callout boxes, as in Callout.ts
...
}`,B0={"shape.domain":S0,"shape.style":E0,"shape.substance":T0,"shape2.style":O0},M0={},G0=`-- acetic acid, expressed via structural-formula DSL
...
AutoLabel All
`,V0=`-- caffeine molecule, expressed via structural-formula DSL
...
`,I0=`-- caffeine molecule, expressed via structural-formula DSL
...
AutoLabel All
`,N0=`-- ethanol molecule, expressed via structural-formula DSL
Carbon C1, C2
...
AutoLabel All
`,_0=`-- glucose molecule, expressed via structural-formula DSL
Oxygen O1
Hydrogen H1, H2, H3, H4
...
`,Q0=`-- methane molecule, expressed via structural-formula DSL
Carbon C1
Hydrogen H1, H2, H3, H4
...
AutoLabel All
`,q0=`-- photosynthesis, expressed via structural-formula DSL
Carbon C1, C2, C3
...
AutoLabel All
`,D0=`-- photosynthesis, expressed via structural-formula DSL
...
AutoLabel All
`,R0=`-- water molecule, expressed via structural-formula DSL
Hydrogen H1, H2
Oxygen O1
...
AutoLabel All
`,j0={"acetic-acid.substance":G0,"caffeine-reduced.substance":V0,"caffeine.substance":I0,"ethanol.substance":N0,"glucose.substance":_0,"methane.substance":Q0,"photosynthesis-half.substance":q0,"photosynthesis.substance":D0,"water.substance":R0},W0=`-- TODO would be really helpful to be able to join multiple
...
`,U0={"methane-combustion.substance":F0,"photosynthesis.substance":H0,"sodium-chlorine.substance":z0},K0=`<?xml version="1.0" encoding="utf-8"?>
...
`,Z0={images:M0,molecules:j0,"pseudo-3d-reaction.style":W0,"pseudo-3d.style":$0,reactions:U0,"structural-formula-atom.svg":K0,"structural-formula.domain":X0,"structural-formula.style":Y0},J0=`-- angle-equivalence.substance
-- 
...
`,aa=`-- length-cross-ratio.substance
-- 
...
HasLabel( k )
HasLabel( l )
`,ia=`-- relative-orientation.substance
-- 
...
AutoLabel All
`,ca=`-- create a triangle with vertices i, j, k
Vertex i, j, k
Edge ij := MakeEdge(i,j)
Edge jk := MakeEdge(j,k)
Edge ki := MakeEdge(k,i)
...
Label j "j"
Label k "k"
`,da={"angle-equivalence.substance":J0,"concyclic-pair.substance":na,"cotan-formula.substance":ea,"edge-label-test.substance":ta,"example.substance":oa,"halfedge-mesh.substance":ra,"length-cross-ratio.substance":aa,"relative-orientation.substance":ia,"segment-intersection-test.substance":la,"triangle-label-test.substance":sa,"triangleCenters.substance":ca},pa=`type Point
type Vertex
type Triangle
...
--- concyclic-pair ---    ensure disjoint( c.textBounds, e.segment )
--- concyclic-pair --- }`,ha={diagrams:da,"triangle-mesh-2d-minimal.domain":pa,"triangle-mesh-2d-minimal.style":ga,"triangle-mesh-2d.domain":ua,"triangle-mesh-2d.style":fa},ba=`type Triangle
`,va=`canvas {
   width = 200
   height = 200
}
...
`,xa={"triangle-mesh-3d.domain":ba,"triangle-mesh-3d.style":va,"two-triangles.substance":ka},ma=`/* This is the starter code for the domain file of tutorial 1,
...
-- End your code above
`,ya=`/* This is the starter code for the style file of tutorial 1,
 * which teaches the basics of Penrose. Follow along with the write-up. 
 * Good luck! :)
 */
...
-- End your code above`,wa=`/* This is the starter code for the substance file of tutorial 1,
 * which teaches the basics of Penrose. Follow along with the write-up. 
 * Good luck! :)
 */
...
-- End your code above
`,Ca={"setTheory.domain":ma,"twoSets.style":ya,"twoSets.substance":wa},La=`/* This is the starter code for the domain program for tutorial 2,
...
-- End your code above
`,Aa=`/* This is the starter code for the style program for tutorial 2,
 * which covers constraints in Penrose. Follow along with the write-up.
 * Good luck! :)
 */
...
`,Pa=`/* This is the starter code for the substance program for tutorial 2,
 * which covers constraints in Penrose. Follow along with the write-up.
 * Good luck! :)
 */
...
-- End your code above
`,Sa={"setTheory.domain":La,"subset.style":Aa,"subset.substance":Pa},Ea=`/* This is the starter code for the domain program for tutorial 3,
...
-- End your code above
`,Ta=`/* This is the starter code for the style program for tutorial 3,
...
-- End your code above
`,Oa=`/* This is the starter code for the substance program for tutorial 3,
 * which covers functions in Penrose. Follow along with the write-up. 
 * Good luck! :)
 */
...
-- End your code above
AutoLabel All /* leave this as the last line */`,Ba={"linearAlgebra.domain":Ea,"vector.style":Ta,"vector.substance":Oa},Ma={tutorial1:Ca,tutorial2:Sa,tutorial3:Ba},Ga={},Va={},Ia={tutorial3:Va},Na={code:Ma,solutions:Ga,supplementary:Ia},_a=`-- Describe a short walk used to estimate the solution to
...
`,Qa=`-- Describe a nested walk used to estimate the solution to,
...
`,qa=`-- Describe a short walk used to estimate the solution to
...
`,Da=`-- Describe a short walk used to estimate the solution to
...
`,Ra=`<svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 240">
...
`,Wa=`<svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 240">
	<rect x="0" y="0" width="320" height="240" fill="none" stroke="none" rx="0" transform="rotate(0, 0, 0)" ensureOnCanvas="true">
		<title>
			Global.box
		</title>
...
`,$a=`<svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 240">
	<rect x="0" y="0" width="320" height="240" fill="none" stroke="none" rx="0" transform="rotate(0, 0, 0)" ensureOnCanvas="true">
		<title>
			Global.box
		</title>
...
`,Fa={"wos-laplace-estimator-walk-on-spheres.svg":Ra,"wos-nested-estimator-walk-on-spheres.svg":ja,"wos-offcenter-estimator-walk-on-spheres.svg":Wa,"wos-poisson-estimator-walk-on-spheres.svg":$a},Ha=`<?xml version="1.0" encoding="utf-8"?>
<!-- Generator: Adobe Illustrator 26.3.1, SVG Export Plug-In . SVG Version: 6.00 Build 0) -->
...
`,Ka={"Laplace.substance":_a,"Nested.substance":Qa,"OffCenter.substance":qa,"Poisson.substance":Da,images:Fa,"walk-on-spheres-ball.svg":Ha,"walk-on-spheres.domain":za,"walk-on-spheres.style":Ua},Xa=`AWord Shells
AWord Minkowski
AWord Exact
AWord Riemannian
AWord BCs
...
`,Za=`-- Set the target diagram size
canvas {
   width = 240
   height = 180
}
...
`,Ja={"example.substance":Xa,"word-cloud.domain":Ya,"word-cloud.style":Za},n={animation:a,"atoms-and-bonds":d,"closest-point":f,"closest-point-ellipse":k,"curve-examples":P,"existential-graph-domain":O,"exterior-algebra":V,"fake-3d-linear-algebra":Q,"fancy-text":j,"full-moon":H,"geometry-domain":Pn,"graph-domain":Jt,"group-theory":lo,"hyperbolic-domain":fo,hypergraph:mo,"lagrange-bases":Ao,"linear-algebra-domain":Io,"logic-circuit-domain":Do,"matrix-ops":ir,"mesh-set-domain":pr,"minkowski-tests":wr,mobius:Pr,molecules:e,"monoidal-category-domain":Or,"penrose-sound":Ir,"persistent-homology":jr,"set-theory-domain":i0,"shape-distance":m0,"shape-spec":P0,"spec-shape-callout":B0,"structural-formula":Z0,"triangle-mesh-2d":ha,"triangle-mesh-3d":xa,tutorials:Na,"walk-on-spheres":Ka,"word-cloud":Ja},ei={domain:"typeppp Set",substancce:"Set A + B",style:`
  Set a {
...
`,variation:""},oi={substance:n["set-theory-domain"]["continuousmap.substance"],style:n["set-theory-domain"]["continuousmap.style"],domain:n["set-theory-domain"]["functions.domain"],variation:""},ri={variation:"ArtemisCrane740",domain:n["exterior-algebra"]["exterior-algebra.domain"],substance:n["exterior-algebra"]["vector-wedge.substance"],style:n["exterior-algebra"]["exterior-algebra.style"]},ai={variation:"MyrtleApe55311",domain:n["linear-algebra-domain"]["linear-algebra.domain"],substance:n["linear-algebra-domain"]["twoVectorsPerp-unsugared.substance"],style:n["linear-algebra-domain"]["linear-algebra-paper-simple.style"]};export{ri as a,oi as c,ei as e,ti as o,ai as v};
//# sourceMappingURL=PenrosePrograms-c74fe919.js.map
