import{m as e}from"./index-e85d8290.js";const t="type Object",o=`canvas {
    width = 400
    height = 400
}
...
AutoLabel All`,a={"animation.domain":t,"animation.style":o,"center-shrink-circle.substance":r},i=`type Atom
type Hydrogen <: Atom
type Oxygen <: Atom
symmetric predicate SBond( Atom a1, Atom a2 )
symmetric predicate WBond( Atom a1, Atom a2 )`,l=`canvas {
    width = 800
    height = 700
}
...
WBond(O1, H3)
WBond(H1, O3)
WBond(H2, O4)`,d={"atoms-and-bonds.domain":i,"atoms-and-bonds.style":l,"one-water-molecule.substance":s,"wet-floor.substance":c},p=`type Point
type Rectangle
type Polygon
type Circle
type Ellipse
...
Ellipse e1
Ellipse e2
`,m={"closest-point-ellipse.domain":h,"closest-point-ellipse.style":b,"closest-point-ellipse.substance":v},k=`Manifold a, b, c
...
forall Curve c {
  ...
  ensure equal(perimeter(c.poly.points, false), length)
  ensure isEquilateral(c.poly)
  encourage equal(elasticEnergy(c.poly.points, false), 0)  -- works better than \`minimal\` in this case
...
`,O={"3blobs.substance":k,"blobs.domain":x,"blobs.style":y,"blobs.substance":C,"closed-elastic-curve.style":w,"cubic-bezier.style":L,"cubic-bezier.substance":P,"curves.domain":S,"homotopy.style":A,"homotopy.substance":E,"open-elastic-curve.style":T},B=`type Graph
type Variable
...
}`,G={"existential-graph.domain":B,"graph.substance":M,"style.style":I},V=`type Scalar
...
`,N=`-- define the size of the drawing
canvas {
   width = 240
   height = 180
}
...
Label u $a \\wedge b$
Label v $b \\wedge c$
`,Q={"exterior-algebra.domain":V,"exterior-algebra.style":N,"vector-wedge.substance":_},D=`type AmbientSpace
type Hyperplane
type Vector
...
AutoLabel All
`,j={"fake3d.domain":D,"fake3d.style":q,"projection.substance":R},W=`type Text
type Point
`,$=`canvas {
   scalar width  = 533.
   scalar height = 300.
...
`,H={"fancy-text.domain":W,"fancy-text.style":$,"fancy-text.substance":F},z=`type Frame
type Moon
type Crater
type Star
type Branch
...
`,X={"full-moon.domain":z,"full-moon.style":U,"full-moon.substance":K},Y=`-- Original Style: https://github.com/penrose/penrose/blob/90e88c5a075d6a75698c49c2feb101275ae64af8/examples/geometry-domain/euclidean.sty
...
-- notation "∠(p, q, r)" ~ "InteriorAngle(p, q, r)"`,en=`-- First construct the right triangle
-- Byrne (1)
Point A, B, C
Triangle ABC := {A, B, C} 
Angle CAB := ∠(C, A, B)
...
AutoLabel All
`,tn=`-- First construct the right triangle
Point A, B, C
Triangle ABC
ABC := MkTriangle(A, B, C) 
...
AutoLabel p, q, r, s, m
Label P $E^2$`,rn=`-- substance name in registry: collinear
Plane R
Point A, B, C, D, E
In(A, R)
In(B, R)
...
Line BA := MkLine(B, A)
Collinear(B,D,E)
AutoLabel A, B, C, D, E, R`,an=`Point M, N, Q, P
...
EqualLength(MN, QP)
AutoLabel M, N, Q, P`,ln=`Point A, B, C, D
Ray DA := MkRay(D, A)
Ray DB := MkRay(D, B)
Ray DC := MkRay(D, C)
...
AutoLabel A, B, C, D`,sn=`-- name in registry: parallel-lines
Point J, K, L, M, N
Line JK := MkLine(J, K)
Line NL := MkLine(N, L)
Line KM := MkLine(K, M)
...
-- Acute(JKM) 
AutoLabel J, K, L, M, N`,cn=`Point A, B, C, D, E
...
AutoLabel A, B, C, D, E
Label aCDE $x$
`,pn=`-- name in registry: midsegment-triangles
Point D, E, F, G, H, J, K
...
AutoLabel D, E, F, G, H, J, K`,gn=`-- name in registry: incenter-triangle
Point J, K, L, P, m
...
AutoLabel J, K, L, M, N`,un=`Point A, B, C, X, Y, Z, P
Triangle ABC := MkTriangle(A, B, C)
Segment AB := MkSegment(A, B)
Segment BC := MkSegment(B, C)
Segment AC := MkSegment(A, C)
...
AutoLabel A, B, C, X, Y, Z, P`,hn=`Point A, B, C, D, E
Line AB := MkLine(A, B)
Line CD := MkLine(C, D)
Parallel(AB, CD)
...
EqualLength(AB, CD)
AutoLabel A, B, C, D, E`,bn=`Point F, G, H, J, K, L
...
EqualAngle(GFL, LFH)
AutoLabel F, G, H, J, K, L`,vn=`Point R, S, T, U, V, W
...
EqualLength(UV, VW)
AutoLabel R, S, T, U, V, W`,mn=`Point P, Q, R, S, T
...
AutoLabel P, Q, R, S, T`,kn=`Point A, B, C, D
Triangle ABC := MkTriangle(A, B, C)
Segment BD := MkSegment(B, D)
Collinear(A, D, C)
AutoLabel All`,xn=`Point J, K, L
Triangle JKL := MkTriangle(J, K, L)
Segment KL := MkSegment(K, L)
Segment j := PerpendicularBisector(KL, J)
AutoLabel All`,yn=`-- TODO one of the triangles crosses with the other segments on a regular basis
Point A, B, C, D, E, F
Triangle ABC := MkTriangle(A, B, C)
...
AutoLabel A, B, C, D, E, F`,Cn=`-- name in registry: circle-example
Point P, Q, U, S, T, R
Circle c
Segment PR := MkSegment(P, R)
...
AutoLabel P, Q, U, S, T, R`,wn=`-- TODO full example can sometimes throw error "2147482504"
...
AutoLabel A, B, C, D, E, F, d, b`,Ln=`Point J, K, L, M, Y
Circle c
Segment LM := Chord(c, L, M)
Segment JK := Chord(c, J, K)
Collinear(J, Y, K)
Collinear(L, Y, M)
AutoLabel All`,Pn=`Point A, B, C, D, O
Circle c
CircleCenter(c, O)
...
Parallel(AB, DC)
AutoLabel A, B, C, D, O`,Sn=`Point T, V, S, P
...
AutoLabel All`,An=`Point A, B, C, D
Triangle ABC := MkTriangle(A, B, C)
...
Parallel(AC, BD)
EqualLength(AC, BD)
AutoLabel A, B, C, D`,En=`-- TODO simplify after attributes are implemented
...
AutoLabel U, S, T, X, Y, Z`,Tn={"c01p01.substance":rn,"c01p10.substance":an,"c02p01.substance":ln,"c03p01.substance":sn,"c04p01.substance":cn,"c04p12.substance":dn,"c05p01.substance":pn,"c05p13.substance":gn,"c06p06.substance":fn,"c07p02.substance":un,"c07p06.substance":hn,"c07p10.substance":bn,"c07p22.substance":vn,"c08p08.substance":mn,"c08p18.substance":kn,"c09p02.substance":xn,"c10p08.substance":yn,"c11p07.substance":Cn,"c11p12.substance":wn,"c11p21.substance":Ln,"c11p25.substance":Pn,"c12p12.substance":Sn,"c12p20.substance":An,"ex.substance":En},On={"euclidean-teaser.style":Y,"euclidean.style":Z,"general-triangle.substance":J,"geometry.domain":nn,"pythagorean-theorem-sugared.substance":en,"pythagorean-theorem-unsugared.substance":tn,"teaser.substance":on,textbook_problems:Tn},Bn=`type Vertex
type Link
...
AutoLabel All
`,Gn=`
Vertex Life, Eucaryote, Bacteria, Archaea, Plantae, Animalia, Fungi, Protista, Cyanobacteria, Proteobacteria, Firmicutes, Actinobacteria, Chlorophyta, Bryophyta, Angiospermae, Porifera, Cnidaria, Mollusca, Arthropoda, Chordata, Ascomycota, Basidiomycota

Edge(Life, Eucaryote)
Edge(Life, Bacteria)
Edge(Life, Archaea)

Edge(Eucaryote, Plantae)
Edge(Eucaryote, Animalia)
Edge(Eucaryote, Fungi)
Edge(Eucaryote, Protista)

Edge(Bacteria, Cyanobacteria)
Edge(Bacteria, Proteobacteria)
Edge(Bacteria, Firmicutes)
Edge(Bacteria, Actinobacteria)

Edge(Plantae, Chlorophyta)
Edge(Plantae, Bryophyta)
Edge(Plantae, Angiospermae)

Edge(Animalia, Porifera)
Edge(Animalia, Cnidaria)
Edge(Animalia, Mollusca)
Edge(Animalia, Arthropoda)
Edge(Animalia, Chordata)

Edge(Fungi, Ascomycota)
Edge(Fungi, Basidiomycota)

AutoLabel All
`,Vn=`
Vertex Glu, Glu6P, Fru6P, Fru16BP, GAP, DHAP, BPG13, PG3, PG2, PEP, Pyr
...
AutoLabel All
`,Nn=`
Vertex AcetylCoA, Citrate, Isocitrate, AlphaKG, SuccinylCoA, Succinate, Fumarate, Malate, Oxaloacetate

Edge(AcetylCoA, Citrate)
Edge(Citrate, Isocitrate)
Edge(Isocitrate, AlphaKG)
Edge(AlphaKG, SuccinylCoA)
Edge(SuccinylCoA, Succinate)
Edge(Succinate, Fumarate)
Edge(Fumarate, Malate)
Edge(Malate, Oxaloacetate)
Edge(Oxaloacetate, Citrate)
...
AutoLabel All
`,Qn={"arpanet.substance":In,"evolutionary-tree.substance":Gn,"glycolysis-pathway.substance":Vn,"krebs-cycle.substance":Nn,"nyc-subway.substance":_n},Dn=`type Vertex
type Link
...
forall Vertex u; Vertex v where Edge(u, v) as e {
  ...
  encourage isEquilateral(e.poly)
...
forall Vertex u; Vertex v where Edge(u, v) as e {
  ...
  ensure lessThan(perimeter(e.poly.points, false), 1.5 * vdist(a, b))
  encourage equal(perimeter(e.poly.points, false), vdist(a, b))
--   encourage minimal(elasticEnergy(e.poly.points, false))
  encourage equal(elasticEnergy(e.poly.points, false), 0)
  encourage isEquilateral(e.poly)
...
Label e "𝑒"
Label f "𝑓"
`,ee=`Vertex Chicago, Denver, Detroit, LA, NYC, SF, Washington
...
Label h "RRP46"
Label i "PM/Sci2"
`,ae=`Vertex team1, team2, team3, team4, team5, team6
...
Label team5 "Team 5"
Label team6 "Team 6"
`,ie=`Vertex Chicago, Denver, Detroit, LA, NYC, SF, Washington
...
Label NYC "New York"
Label SF "San Francisco"
`,le=`Vertex Chicago, Denver, Detroit, LA, NYC, SF, Washington
...
Label NYC "New York"
Label SF "San Francisco"
`,se=`Vertex Chicago, Denver, Detroit, LA, NYC, SF, Washington
...
Label NYC "New York"
Label SF "San Francisco"
`,ce=`Vertex Chicago, Denver, Detroit, LA, NYC, SF, Washington
...
AutoLabel All
`,he={"ex3.substance":Un,"ex4.substance":Kn,"ex5.substance":Xn,"ex6.substance":Yn,"ex7.substance":Zn,"ex8.substance":Jn,"ex9.substance":ne,"fig1.substance":ee,"fig10.substance":te,"fig11.substance":oe,"fig12.substance":re,"fig13.substance":ae,"fig2.substance":ie,"fig3.substance":le,"fig4.substance":se,"fig5.substance":ce,"fig6.substance":de,"fig7.substance":pe,"fig8a.substance":ge,"fig8b.substance":fe,"fig9.substance":ue},be=`Vertex a, b, c, d, e, f
...
Label d "𝑑"
Label e "𝑒"
`,Te=`Vertex Alvarez, Berkowitz, Chen, Davis
...
AutoLabel All
`,Oe=`Vertex Washington, Xuan, Ybarra, Ziegler
Vertex requirements, architecture, implementation, testing
...
Label p32 "𝑃(3, 2)"
Label p33 "𝑃(3, 3)"
`,Ne=`Vertex p0, p1, p2, p3, p4, p5, p6, p7
...
Edge(k26t2, k26b5)
Edge(k26t2, k26b6)
`,Ue={"eg9.substance":be,"ex1.substance":ve,"ex2.substance":me,"ex21.substance":ke,"ex22.substance":xe,"ex23.substance":ye,"ex24.substance":Ce,"ex25.substance":we,"ex3.substance":Le,"ex51.substance":Pe,"ex7.substance":Se,"ex8.substance":Ae,"ex9.substance":Ee,"fig10a.substance":Te,"fig10b.substance":Oe,"fig11a.substance":Be,"fig11b.substance":Me,"fig11c.substance":Ie,"fig12.substance":Ge,"fig13.substance":Ve,"fig14.substance":Ne,"fig16b.substance":_e,"fig1G.substance":Qe,"fig1H.substance":De,"fig2.substance":qe,"fig3.substance":Re,"fig4.substance":je,"fig5.substance":We,"fig6.substance":$e,"fig8G.substance":Fe,"fig8H.substance":He,"fig9.substance":ze},Ke=`Vertex a, b, c, d
...
Label v4 "𝑣₄"
Label v5 "𝑣₅"
`,lt=`Vertex u1, u2, u3, u4, u5, u6, u7
...
Label v5 "𝑣₅"
Label v6 "𝑣₆"
`,gt=`Vertex u1, u2, u3, u4, u5, u6, u7, u8
...
Label v7 "𝑣₇"
Label v8 "𝑣₈"
`,ft=`Vertex u1, u2, u3, u4, u5, u6, u7, u8, u9, u10
...
Label v9 "𝑣₉"
Label v10 "𝑣₁₀"
`,ut=`Vertex u1, u2, u3, u4, u5, u6, u7, u8, u9, u10
...
Label v9 "𝑣₉"
Label v10 "𝑣₁₀"
`,ht=`Vertex u1, u2, u3, u4, u5, u6, u7, u8
...
Label v3 "𝑣₃"
Label v4 "𝑣₄"
`,Ot={"ex1.substance":Ke,"ex13.substance":Xe,"ex14.substance":Ye,"ex15.substance":Ze,"ex19.substance":Je,"ex2.substance":nt,"ex20.substance":et,"ex21.substance":tt,"ex3.substance":ot,"ex34.substance":rt,"ex35.substance":at,"ex36.substance":it,"ex37.substance":lt,"ex38.substance":st,"ex39.substance":ct,"ex4.substance":dt,"ex40.substance":pt,"ex41.substance":gt,"ex42.substance":ft,"ex43.substance":ut,"ex44.substance":ht,"ex50.substance":bt,"ex61.substance":vt,"ex62.substance":mt,"ex63.substance":kt,"ex64.substance":xt,"fig1.substance":yt,"fig10.substance":Ct,"fig11.substance":wt,"fig12.substance":Lt,"fig2.substance":Pt,"fig3.substance":St,"fig4.substance":At,"fig5.substance":Et,"fig8.substance":Tt},Bt=`Vertex a, b, c, d, e
...
Label f "𝑓"
Label g "𝑔"
`,_t={"ex11a.substance":Bt,"ex11b.substance":Mt,"ex11c.substance":It,"ex12a.substance":Gt,"ex12b.substance":Vt,"ex12c.substance":Nt},Qt=`Vertex a, b, c, d
...
Label f "𝑓"
Label g "𝑔"
`,$t=`Vertex a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q
...
Edge(d, f)
Edge(e, f)
`,Zt={"ex18.substance":Qt,"ex21.substance":Dt,"ex30.substance":qt,"ex31.substance":Rt,"ex32.substance":jt,"ex33.substance":Wt,"ex34.substance":$t,"ex35.substance":Ft,"ex36.substance":Ht,"ex46.substance":zt,"ex47a.substance":Ut,"ex47b.substance":Kt,"ex47c.substance":Xt,"ex47d.substance":Yt},Jt=`Vertex a, b, c, d, e, f
...
Edge(h, g)
Edge(e, g)
`,eo={"ex5.substance":Jt,"fig4.substance":no},to={sec1:he,sec2:Ue,sec3:Ot,sec4:_t,sec5:Zt,sec7:eo},oo={"directed-multigraph.domain":Bn,"directed-multigraph.style":Mn,"other-examples":Qn,"pseudograph.domain":Dn,"pseudograph.style":qn,"simple-curved-graph-boxes.style":Rn,"simple-curved-graph-dots.style":jn,"simple-curved-graph.style":Wn,"simple-directed-graph.domain":$n,"simple-directed-graph.style":Fn,"simple-graph.domain":Hn,"simple-graph.style":zn,textbook:to},ro=`-- Draws a Cayley graph for a group G, assuming that
...
`,ao=`type Element -- an element of the group
...
Label g9 $g_{9}$
Label g10 $g_{10}$
`,co=`-- variation: LeverkaasAlbatross41817
Element g1
Element g2
Element g3
Element g4
...
Label g7 $-j$
Label g8 $-k$
`,po={"dihedral-1.substance":lo,"dihedral-5.substance":so,"quaternions.substance":co},go={"CayleyGraph.style":ro,"Group.domain":ao,"MultiplicationTable.style":io,groups:po},fo={},uo=`canvas {
   scalar width  = 533.
   scalar height = 300.
}
...
`,mo={Notes:fo,"PoincareDisk.style":uo,"hyperbolic-example-unsugared.substance":ho,"hyperbolic-example.substance":bo,"hyperbolic.domain":vo},ko=`<svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 700">
  <rect
    x="291.702230696695"
    y="345.9981178395118"
    width="20"
...
`,Lo={"example.svg":ko,"hypergraph-3d.style":xo,"hypergraph.domain":yo,"hypergraph.style":Co,"hypergraph.substance":wo},Po=`Node a, b, c, d
Node u, v, x, y, z
...
`,So=`<svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 240 180">
  <rect
    x="0"
    y="0"
    width="240"
...
`,To={"example.substance":Po,"example.svg":So,"lagrange-bases.domain":Ao,"lagrange-bases.style":Eo},Oo=`VectorSpace V
Vector u 
Vector v 
In(u, V)
In(v, V)
...
Orthogonal(x1, x2)
AutoLabel All
`,Do={"dependent.substance":Oo,"error-test.style":Bo,"independent.substance":Mo,"linear-algebra-paper-simple.style":Io,"linear-algebra-test.domain":Go,"linear-algebra.domain":Vo,"linearalgebra-paper-dashes.style":No,"twoVectorsPerp-unsugared.substance":_o,"twoVectorsPerp.substance":Qo},qo=`canvas {
    width = 800
    height = 700
}
...
Connection C16 := MakeConnection(XOR3OUT, CARRY)`,jo=`AutoLabel All
...
Connection C8 := MakeConnection(ANDOUT, CAR)`,Wo=`-- Nodes
...
constructor MakeConnection(Node A, Node B) -> Connection`,$o={"distinctive-shape.style":qo,"full-adder.substance":Ro,"half-adder.substance":jo,"logic-gates.domain":Wo},Fo=`type Scalar
type Vector
type Matrix
...
`,cr={"matrix-matrix-addition.substance":zo,"matrix-matrix-division-elementwise.substance":Uo,"matrix-matrix-multiplication-elementwise.substance":Ko,"matrix-matrix-multiplication.substance":Xo,"matrix-matrix-subtraction.substance":Yo,"matrix-transpose.substance":Zo,"matrix-vector-left-multiplication.substance":Jo,"matrix-vector-right-multiplication.substance":nr,"scalar-vector-division.substance":er,"scalar-vector-left-multiplication.substance":tr,"scalar-vector-right-multiplication.substance":or,"vector-vector-addition.substance":rr,"vector-vector-division-elementwise.substance":ar,"vector-vector-multiplication-elementwise.substance":ir,"vector-vector-outerproduct.substance":lr,"vector-vector-subtraction.substance":sr},dr={"matrix-ops.domain":Fo,"matrix-ops.style":Ho,tests:cr},pr=`-- Clashing notations with \\in symbol
...
predicate Identified(Type, Type)`,fr=`-- Random seeds; first seed controls the vertex selection, second seed controls the mesh topology
...
}`,ur=`-- Clashing notations with \\in symbol
-- Vertex v1, v2, v3, v4 -- ∈ K (can't say this)
...
-- Also identify faces with sets
Identified(f, A)
`,hr={"DomainInterop-unsugared.substance":pr,"DomainInterop.domain":gr,"DomainInterop.style":fr,"DomainInterop.substance":ur},br=`--- This .style file is intended for debugging purposes only
...
`,vr=`--- This .style file is intended for debugging purposes only
...
Maze M
Triangle T1, T2, T3
`,yr={"ellipse-maze.style":br,"ellipses-maze.style":vr,"maze.domain":mr,"maze.style":kr,"maze.substance":xr},Cr=`
type Shape
`,wr=`
canvas {
  width = 800
...
`,Lr=`
Shape Yin, Yang
`,Pr={"yin-yang.domain":Cr,"yin-yang.style":wr,"yin-yang.substance":Lr},Sr={maze:yr,"yin-yang":Pr},Ar=`type Center
type Shape
...
constructor Inversion( Shape original, Center center ) -> Shape`,Er=`canvas {
    width = 800
    height = 700
}
...
Label ic1 $c'_1$
Label ic2 $c'_2$
Label ic3 $c'_3$`,Or={"mobius.domain":Ar,"mobius.style":Er,"mobius.substance":Tr},Br=`type Object
type Morphism
...
constructor join(Object first, Object second) -> Morphism`,Mr=`Colors {
...
-- AutoLabel All`,Gr={"monoidal-test.domain":Br,"monoidal-test.style":Mr,"test.substance":Ir},Vr=`<!-- <svg width="100%" height="100%" version="1.2" viewBox="0 0 240 180" xmlns="http://www.w3.org/2000/svg"> -->
...
`,Qr=`-- define the size of the drawing
canvas {
   width = 240
   height = 180
}
...
`,Dr={"penrose-sound-example.svg":Vr,"sound-test.substance":Nr,"sound.domain":_r,"sound.style":Qr},qr=`<svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 700">
  <circle
    fill="#339933"
    fill-opacity="0.3"
...
`,Hr={"example.svg":qr,"persistent-homology-basic.style":Rr,"persistent-homology-shading.svg":jr,"persistent-homology.domain":Wr,"persistent-homology.style":$r,"persistent-homology.substance":Fr},zr=`type Point
...
`,Kr=`-- domain: random-sampling.domain
...
`,Xr={"random-sampling.domain":zr,"random-sampling.style":Ur,"test-random-sampling.substance":Kr},Yr=`canvas {
  width = 800
  height = 700
}
...
AutoLabel All
`,t0=`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 874.58 820.84">
  <defs>
...
predicate Intersecting(Set s1, Set s2)
predicate IsSubset(Set s1, Set s2)
`,r0=`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 829.57 829.57">
  <defs>
...
`,c0=`-- Fork of venn-opt-test to verify doing optimization through an (external) computation
...
`,u0={"continuousmap.style":Yr,"continuousmap.substance":Zr,"functions.domain":Jr,"multisets.substance":n0,"nested.substance":e0,"set-theory-domain-shadow.svg":t0,"setTheory.domain":o0,"shading.svg":r0,"tree.style":a0,"tree.substance":i0,"twosets-simple.substance":l0,"venn-3d.style":s0,"venn-comp-test.style":c0,"venn-polygon.style":d0,"venn-small.style":p0,"venn-test.style":g0,"venn.style":f0},h0=`canvas {
  width = 800
  height = 700
}
...
}`,b0=`Point p1, p2, p3, p4, p5, p6, p7, p8
Polygon g
...
e2 := MkEdge(b1, b2)
e3 := MkEdge(b1, b2)
e4 := MkEdge(b1, b2)`,E0={"box-arrow.style":h0,"eight-points-around-polygon.substance":b0,"eighty-text.substance":v0,"ellipse.substance":m0,"hundred-points-around-line.substance":k0,"hundred-points-around-star.substance":x0,"lines-around-rect.substance":y0,"ninety-points-around-polyline.substance":C0,"rect-line-dist.style":w0,"shape-distance.style":L0,"shapes.domain":P0,"supersimplegraph.domain":S0,"two-nodes-two-edges.substance":A0},T0=`type Shape
type Circle <: Shape
type Ellipse <: Shape
...
`,G0={"all-shapes.domain":T0,"all-shapes.substance":O0,"arrows.style":B0,"arrows.substance":M0,"shape-spec.style":I0},V0=`type Node
`,N0=`-- Visualize the construction of callout boxes, as in Callout.ts
...
}`,D0={"shape.domain":V0,"shape.style":N0,"shape.substance":_0,"shape2.style":Q0},q0={},R0=`-- acetic acid, expressed via structural-formula DSL
...
AutoLabel All
`,j0=`-- caffeine molecule, expressed via structural-formula DSL
...
`,W0=`-- caffeine molecule, expressed via structural-formula DSL
...
AutoLabel All
`,$0=`-- ethanol molecule, expressed via structural-formula DSL
Carbon C1, C2
...
AutoLabel All
`,F0=`-- glucose molecule, expressed via structural-formula DSL
Oxygen O1
Hydrogen H1, H2, H3, H4
...
`,H0=`-- methane molecule, expressed via structural-formula DSL
Carbon C1
Hydrogen H1, H2, H3, H4
...
AutoLabel All
`,z0=`-- photosynthesis, expressed via structural-formula DSL
Carbon C1, C2, C3
...
AutoLabel All
`,U0=`-- photosynthesis, expressed via structural-formula DSL
...
AutoLabel All
`,K0=`-- water molecule, expressed via structural-formula DSL
Hydrogen H1, H2
Oxygen O1
...
AutoLabel All
`,X0={"acetic-acid.substance":R0,"caffeine-reduced.substance":j0,"caffeine.substance":W0,"ethanol.substance":$0,"glucose.substance":F0,"methane.substance":H0,"photosynthesis-half.substance":z0,"photosynthesis.substance":U0,"water.substance":K0},Y0=`-- TODO would be really helpful to be able to join multiple
...
`,ta={"methane-combustion.substance":J0,"photosynthesis.substance":na,"sodium-chlorine.substance":ea},oa=`<?xml version="1.0" encoding="utf-8"?>
...
`,ia={images:q0,molecules:X0,"pseudo-3d-reaction.style":Y0,"pseudo-3d.style":Z0,reactions:ta,"structural-formula-atom.svg":oa,"structural-formula.domain":ra,"structural-formula.style":aa},la=`
...
`,sa=`-- angle-equivalence.substance
-- 
...
`,ua=`-- length-cross-ratio.substance
-- 
...
HasLabel( k )
HasLabel( l )
`,ha=`-- relative-orientation.substance
-- 
...
AutoLabel All
`,ma=`-- create a triangle with vertices i, j, k
Vertex i, j, k
Edge ij := MakeEdge(i,j)
Edge jk := MakeEdge(j,k)
Edge ki := MakeEdge(k,i)
...
Label j "j"
Label k "k"
`,ka={"angle-equivalence.substance":sa,"concyclic-pair.substance":ca,"cotan-formula.substance":da,"edge-label-test.substance":pa,"example.substance":ga,"halfedge-mesh.substance":fa,"length-cross-ratio.substance":ua,"relative-orientation.substance":ha,"segment-intersection-test.substance":ba,"triangle-label-test.substance":va,"triangleCenters.substance":ma},xa=`type Point
type Vertex
type Triangle
...
--- concyclic-pair ---    ensure disjoint( c.textBounds, e.segment )
--- concyclic-pair --- }`,La={"concyclic.style":la,diagrams:ka,"triangle-mesh-2d-minimal.domain":xa,"triangle-mesh-2d-minimal.style":ya,"triangle-mesh-2d.domain":Ca,"triangle-mesh-2d.style":wa},Pa=`type Triangle
`,Sa=`canvas {
   width = 200
   height = 200
}
...
`,Ea={"triangle-mesh-3d.domain":Pa,"triangle-mesh-3d.style":Sa,"two-triangles.substance":Aa},Ta=`/* This is the starter code for the domain file of tutorial 1,
...
-- End your code above
`,Oa=`/* This is the starter code for the style file of tutorial 1,
 * which teaches the basics of Penrose. Follow along with the write-up. 
 * Good luck! :)
 */
...
-- End your code above`,Ba=`/* This is the starter code for the substance file of tutorial 1,
 * which teaches the basics of Penrose. Follow along with the write-up. 
 * Good luck! :)
 */
...
-- End your code above
`,Ma={"setTheory.domain":Ta,"twoSets.style":Oa,"twoSets.substance":Ba},Ia=`/* This is the starter code for the domain program for tutorial 2,
...
-- End your code above
`,Ga=`/* This is the starter code for the style program for tutorial 2,
 * which covers constraints in Penrose. Follow along with the write-up.
 * Good luck! :)
 */
...
`,Va=`/* This is the starter code for the substance program for tutorial 2,
 * which covers constraints in Penrose. Follow along with the write-up.
 * Good luck! :)
 */
...
-- End your code above
`,Na={"setTheory.domain":Ia,"subset.style":Ga,"subset.substance":Va},_a=`/* This is the starter code for the domain program for tutorial 3,
...
-- End your code above
`,Qa=`/* This is the starter code for the style program for tutorial 3,
...
-- End your code above
`,Da=`/* This is the starter code for the substance program for tutorial 3,
 * which covers functions in Penrose. Follow along with the write-up. 
 * Good luck! :)
 */
...
-- End your code above
AutoLabel All /* leave this as the last line */`,qa={"linearAlgebra.domain":_a,"vector.style":Qa,"vector.substance":Da},Ra={tutorial1:Ma,tutorial2:Na,tutorial3:qa},ja={},Wa={},$a={tutorial3:Wa},Fa={code:Ra,solutions:ja,supplementary:$a},Ha=`-- Describe a short walk used to estimate the solution to
...
`,za=`-- Describe a nested walk used to estimate the solution to,
...
`,Ua=`-- Describe a short walk used to estimate the solution to
...
`,Ka=`-- Describe a short walk used to estimate the solution to
...
`,Xa=`<svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 240">
...
`,Ya=`<svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 240">
	<rect x="0" y="0" width="320" height="240" fill="none" stroke="none" rx="0" transform="rotate(0, 0, 0)" ensureOnCanvas="true">
		<title>
			Global.box
		</title>
...
`,Za=`<svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 240">
	<rect x="0" y="0" width="320" height="240" fill="none" stroke="none" rx="0" transform="rotate(0, 0, 0)" ensureOnCanvas="true">
		<title>
			Global.box
		</title>
...
`,Ja=`<svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 240">
	<rect x="0" y="0" width="320" height="240" fill="none" stroke="none" rx="0" transform="rotate(0, 0, 0)" ensureOnCanvas="true">
		<title>
			Global.box
		</title>
...
`,ni={"wos-laplace-estimator-walk-on-spheres.svg":Xa,"wos-nested-estimator-walk-on-spheres.svg":Ya,"wos-offcenter-estimator-walk-on-spheres.svg":Za,"wos-poisson-estimator-walk-on-spheres.svg":Ja},ei=`<?xml version="1.0" encoding="utf-8"?>
<!-- Generator: Adobe Illustrator 26.3.1, SVG Export Plug-In . SVG Version: 6.00 Build 0) -->
...
`,ri={"Laplace.substance":Ha,"Nested.substance":za,"OffCenter.substance":Ua,"Poisson.substance":Ka,images:ni,"walk-on-spheres-ball.svg":ei,"walk-on-spheres.domain":ti,"walk-on-spheres.style":oi},ai=`AWord Shells
AWord Minkowski
AWord Exact
AWord Riemannian
AWord BCs
...
`,li=`-- Set the target diagram size
canvas {
   width = 240
   height = 180
}
...
`,si={"example.substance":ai,"word-cloud.domain":ii,"word-cloud.style":li},n={animation:a,"atoms-and-bonds":d,"closest-point":u,"closest-point-ellipse":m,"curve-examples":O,"existential-graph-domain":G,"exterior-algebra":Q,"fake-3d-linear-algebra":j,"fancy-text":H,"full-moon":X,"geometry-domain":On,"graph-domain":oo,"group-theory":go,"hyperbolic-domain":mo,hypergraph:Lo,"lagrange-bases":To,"linear-algebra-domain":Do,"logic-circuit-domain":$o,"matrix-ops":dr,"mesh-set-domain":hr,"minkowski-tests":Sr,mobius:Or,molecules:e,"monoidal-category-domain":Gr,"penrose-sound":Dr,"persistent-homology":Hr,"random-sampling":Xr,"set-theory-domain":u0,"shape-distance":E0,"shape-spec":G0,"spec-shape-callout":D0,"structural-formula":ia,"triangle-mesh-2d":La,"triangle-mesh-3d":Ea,tutorials:Fa,"walk-on-spheres":ri,"word-cloud":si},di={domain:"typeppp Set",substancce:"Set A + B",style:`
  Set a {
...
`,variation:""},gi={substance:n["set-theory-domain"]["continuousmap.substance"],style:n["set-theory-domain"]["continuousmap.style"],domain:n["set-theory-domain"]["functions.domain"],variation:""},fi={variation:"ArtemisCrane740",domain:n["exterior-algebra"]["exterior-algebra.domain"],substance:n["exterior-algebra"]["vector-wedge.substance"],style:n["exterior-algebra"]["exterior-algebra.style"]},ui={variation:"MyrtleApe55311",domain:n["linear-algebra-domain"]["linear-algebra.domain"],substance:n["linear-algebra-domain"]["twoVectorsPerp-unsugared.substance"],style:n["linear-algebra-domain"]["linear-algebra-paper-simple.style"]};export{fi as a,gi as c,di as e,pi as o,ui as v};
//# sourceMappingURL=PenrosePrograms-28ddb5f1.js.map
