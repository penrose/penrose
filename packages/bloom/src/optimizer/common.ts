...
/** Minimum state for compilation */
export type MinState = {
  canvas: Canvas;
  variation: string;
  inputs: Var[];
  inputInits: ("Pending" | "Sampled")[];
  constraints: Num[];
  objectives: Num[];
  shapes: Shape<Num>[];
};
...
export enum MessageTags {
  ...
  LabelMeasurements = "LabelMeasurements",
  Resample = "Resample",
  CompileFromMinState = "CompileFromMinState",
}
...
export type CompileFromMinStateResult = TaggedResult<
  DiagramID,
  PenroseError,
  MessageTags.CompileFromMinState
>;
