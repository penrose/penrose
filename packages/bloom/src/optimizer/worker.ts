...
const historyInfo = new SizeBoundedMap<StepSequenceID, StepSequenceInfo>(
...
const compileFromMinState = async (
  ...
  const state: PenroseState = {
    warnings: [],
    variation: data.minState.variation,
    constraintSets,
    objFns: [],
    constrFns: [],
    ...
    canvas: data.minState.canvas,
    currentStageIndex: 0,
    optStages: [""],
    params: start(data.minState.inputs.length),
    gradient: await genGradient(
      data.minState.inputs,
      data.minState.objectives,
      data.minState.constraints,
    ),
    computeShapes: await compileCompGraph(
      data.minState.inputs,
      data.minState.shapes,
    ),
  };
    ...
    case "MessageRequest":
      {
        ...
        switch (requestData.tag) {
          ...
          case MessageTags.CompileFromMinState:
            result = await compileFromMinState(requestData);
            break;
