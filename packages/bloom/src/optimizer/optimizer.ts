...
export default class Optimizer {
  ...
  static create = async (): Promise<Optimizer> => {
    ...
    return new Optimizer(await launchAndWaitForInit(new Broker()));
  };

  private computeLayoutAndSendMeasurements = async (
    diagramId: DiagramID,
  ): Promise<Result<RenderState, PenroseError>> => {
    const layoutResult = await this.computeLayout(diagramId, {
      sequenceId: 0,
      frame: 0,
    });
  ...
  compileFromMinimumState = async (
    ...
    const diagramId = result.value;
    const notifyResult = await this.computeLayoutAndSendMeasurements(diagramId);
    if (notifyResult.isErr()) {
      return taggedErr(MessageTags.CompileFromMinState, notifyResult.error);
    }
