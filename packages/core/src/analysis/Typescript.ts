 ...
 *  - This analysis is neither complete nor has it been proven sound.
 ...
 *  - This analysis does not find functions nested within other functions.
 *  - If a function cannot be found, an exception is thrown to avoid unsoundness.
 *
...
export const tsAnalyzePropertyAccess = (
  ...
  // Pending indicator - to avoid infinite recursion.
  ...
  /**
   * Behind the scenes function that implements tsAnalyzePropertyAccess
   */
  ...
  return tsAnalyzePropertyAccessImpl(fnName, arg);
 ...
 * Functions, Objectives, Constraints, and finally the list of
 * non-exported functions in Functions.
 *
...
const getTsFnSource = (fnName: string): string => {
  ...
  else if (fnPath[0] === "ops" && fnPath[1] in ops)
    return "const f = " + ops[fnPath[1]].toString();
  //
