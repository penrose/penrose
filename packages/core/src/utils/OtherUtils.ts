...
export const prettyPrintPath = (p: Expr): string => {
  ...
  if (p.tag === "FieldPath") {
    ...
    return varName + DELIM + varField;
  } else if (p.tag === "PropertyPath") {
    const varName = p.name.contents.value;
    const varField = p.field.value;
    const property = p.property.value;
    return varName + DELIM + varField + DELIM + property;
...
// Used because _.partition is too slow for opt... https://stackoverflow.com/questions/51916919/js-lodash-partition-by-a-array-of-predicates
export function partition<T>(xs: T[], f: (x: T) => boolean): [T[], T[]] {
  const output: [T[], T[]] = [[], []];
  for (let i = 0; i < xs.length; i++) {
    const e = xs[i];
    if (f(e)) {
