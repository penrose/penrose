...
describe("polyRoots tests", () => {
  test("degree 1", async () => {
    ...
    const v = variable(x);
    const [z] = polyRoots([v]);
    const f = await makeFunc({ inputs: [v], output: z });
  ...
  const testQuadratic = async (f1: F, f2: F) => {
    const x1 = Math.PI;
    const x2 = Math.E;
    ...
    const closedForm = await makeFunc({
      inputs: [b, c],
      // c + bx + ax²
      output: div(
    ...
    const implicit = await makeFunc({ inputs: [b, c], output: f2(r1, r2) });
  ...
  test("quadratic formula min root", () => testQuadratic(sub, min));

  test("quadratic formula max root", () => testQuadratic(add, max));

  test("cubic with only one real root", async () => {
    ...
    const f = await makeFunc({ inputs, output: z });
