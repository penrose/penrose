...
describe("problem tests", () => {
  ...
  test("no freeze", async () => {
    ...
    for (const [z, a] of run.vals) z.assign(scalar(a));
    expect(x.arraySync()).toBeCloseTo(0);
    expect(y.arraySync()).toBeCloseTo(0);
  });
  ...
  test("freeze", async () => {
    ...
    expect(run.converged).toBe(true);
    for (const [z, a] of run.vals) z.assign(scalar(a));
    expect(x.arraySync()).toBe(1);
    expect(y.arraySync()).toBeCloseTo(1);
  });
});
