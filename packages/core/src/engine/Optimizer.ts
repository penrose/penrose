...
export const step = (state: State, steps: number, evaluate = true) => {
    ...
    switch (optStatus) {
        ...
        case "UnconstrainedRunning": {
            ...
            newState.optDebugInfo = optDebugInfo;
...
const minimize = (
    ...
    let optDebugInfo: OptStateInfo = {
        ...
        normGrad: -1,
        preconditionedGrad: [],
        preconditionedGradNorm: -1,
        lineSearchStepSize: -1,
        pathMap: [] as [string, number, number, number][]
    };
    ...
    while (i < numSteps) {
        ...
        preconditionedGradNorm = Math.sqrt(dot(gradfxs, gradfxsPreconditioned));

        if (BREAK_EARLY && unconstrainedConverged2(preconditionedGradNorm)) {
        ...
        const pathMap = _.zip(varyingPaths, xs, gradfxs, gradientPreconditioned) as [
            string,
            number,
            number,
            number
        ...
        optDebugInfo = {
            ...
            grad: gradfxs,
            normGrad,
            preconditionedGrad: gradientPreconditioned,
            preconditionedGradNorm,
            lineSearchStepSize: t,
            pathMap
        };
        ...
        if (DEBUG_GRAD_DESCENT) {
            ...
            log.info("preconditioned grad (grad(f)(xs)):", gradientPreconditioned);
            log.info("sqrt(<grad, preconditioned grad>):", preconditionedGradNorm);
            log.info("[varying paths, current val, gradient of val, gradientPreconditioned]", pathMap);
            log.info("t", t);
        }
    ...
    return {
        xs,
        energyVal: fxs,
        normGrad: preconditionedGradNorm,
        newLbfgsInfo,
        gradient: gradfxs,
        gradientPreconditioned,
        optDebugInfo
