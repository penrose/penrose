...
const pruneOptProblem = (s: State): any => {
  ...
  // Ignore all non-disjoint functions -- this only needs to be done once
  let [allRelevantConstrs, remainingConstrs]: [Fn[], Fn[]] = [[], []];

  // First time: cache it
  if (
    !params.allRelevantConstrs ||
    (params.allRelevantConstrs.length + params.remainingConstrs.length === 0 &&
      s.objFns.length + s.constrFns.length !== 0)
  ) {
    [allRelevantConstrs, remainingConstrs] = partition(
      s.constrFns,
      (f) => f.fname === "disjoint" && f.optType === "ConstrFn"
    );
    params.allRelevantConstrs = allRelevantConstrs;
    params.remainingConstrs = remainingConstrs;
  } else {
    // Retrieve cached
    [allRelevantConstrs, remainingConstrs] = [
      params.allRelevantConstrs,
      params.remainingConstrs,
    ];
  }
  ...
  const shapes = s.shapes.filter(
    (o) => o.properties.name.contents !== CONTAINER && o.shapeType !== "Text"
  );
  ...
  for (let i = 0; i < shapes.length; i++) {
    ...
    const cell = [
      Math.floor((center[0] + GRID_DIMS.w / 2) / GRID_CELL_DIM),
  ...
  const plus2 = (v: number[], w: number[]): number[] => [
    v[0] + w[0],
    v[1] + w[1],
  ]; // TODO: import the right util
  const includesList = (
    xss: [string, string][],
    ys: [string, string]
  ): boolean => _.some(xss, (xs) => xs[0] === ys[0] && xs[1] === ys[1]); // TODO make this parametric. `.includes` doesn't work with objects

  // For shape A, look at its neighbors Bi in the grid.
  for (let i = 0; i < shapes.length; i++) {
    const cell = cells[i] as number[];
    // Ignore out-of-bounds ones
    if (cell[0] === undefined || cell[1] === undefined) {
      continue;
    }

    const name = shapes[i].properties.name.contents;
    const neighboringShapes: string[] = _.flatten(
      [
        [1, 1],
        [1, 0],
        [1, -1],
    ...
    const neighborPairs: [string, string][] = neighboringShapes.map((n) =>
      ([name, n] as [string, string]).sort()
    ); // Sort for comparison to `disjoint` args

    // TODO: Actually, not sure how to filter out the list of relevant constraints by the neighbors w/o another n^2 iteration? Unless they are generated?
    let cellRelevantConstrs = [];
    [cellRelevantConstrs, allRelevantConstrs] = partition(
      allRelevantConstrs,
      (f) => {
        const args = f.fargs.map(prettyPrintExpr);
        if (args.length < 2) throw Error("expected 2+ args");
        let argsSorted: [string, string] = args.slice(0, 2).sort() as [
          string,
          string
        ]; // For comparison
  ...
  const compiledConstrs: FnCached[] = prunedConstrFns.map(
    (f) => params.constrFnCache[prettyPrintFn(f)]
  );
  const compiledObjectives: FnCached[] = objectiveFns.map(
    (f) => params.objFnCache[prettyPrintFn(f)]
  );
  ...
  const prunedGradient = (zs: number[]): number[] => {
    ...
    const gradEnergies = compiledObjectives.length
      ? vec.vaddList(compiledObjectives.map((e) => e.gradf(zs)))
      : init;
    const gradConstrEnergies = compiledConstrs.length
      ? vec.vaddList(compiledConstrs.map((e) => e.gradf(zs)))
      : init;
    return vec.vadd(
      gradEnergies,
      vec.vmul(constraintWeight * s.params.weight, gradConstrEnergies)
    );
  };
  ...
  switch (optStatus) {
    ...
    case "UnconstrainedRunning": {
      ...
      if (USE_PRUNED) {
        ...
        console.log(
          `Time to prune for ~${state.shapes.length} shapes and ${
            state.objFns.length + state.constrFns.length
          } objectives/constraints: ${t1 - t0} ms`
        );

        [energyToUse, gradientToUse] = [prunedObjective, prunedGradient];
      } else {
        [energyToUse, gradientToUse] = [
      ...
      console.log(
        `Performance: times (ms for each 1 call to minimize) over ${ts.length} samples`
      );
...
const minimize = (
  ...
  console.log(
    `Time taken for 1 call to 'minimize' with ${numSteps} steps: ${t1 - t0} ms`
  );
