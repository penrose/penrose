...
// math.js types aren't always precise enough
const checkMatrix = (m: math.MathType): math.Matrix => {
  if (typeof m !== "number" && "type" in m) {
    return m;
  } else {
    throw Error("math.js returned something other than a Matrix");
  }
};
...
const vecList = (xs: math.Matrix): number[] => {
  // Prints a col vector (nx1)
  const res = [];
  for (let i = 0; i < xs.size()[0]; i++) {
    res.push(xs.get([i, 0]));
  }
  return res;
};

const printVec = (xs: math.Matrix) => {
  // Prints a col vector (nx1)
  log.info("xs (matrix)", vecList(xs));
};

const colVec = (xs: number[]): math.Matrix => {
  // Return a col vector (nx1)
  // TODO: What is the performance of this?
  const m = checkMatrix(math.zeros(xs.length, 1)); // rows x cols
...
const lbfgsInner = (
  ...
  // Helper functions
  const calculate_rho = (s: math.Matrix, y: math.Matrix): number => {
    return 1.0 / (math.dot(y, s) + EPSD);
  };

  const pull_q_back = (
    acc: [math.Matrix, number[]],
    curr: [number, math.Matrix, math.Matrix]
  ): [math.Matrix, number[]] => {
    const [q_i_plus_1, alphas2] = acc; // alphas2 is the same stuff as alphas, just renamed to avoid shadowing
    const [rho_i, s_i, y_i] = curr;

    const alpha_i: number = rho_i * math.dot(s_i, q_i_plus_1);
    const q_i: math.Matrix = checkMatrix(
      math.subtract(q_i_plus_1, math.multiply(y_i, alpha_i))
    );
  ...
  const estimate_hess = (
    ...
    const gamma_k = math.dot(s_km1, y_km1) / (math.dot(y_km1, y_km1) + EPSD);
    const [n] = y_km1.size();
    return math.multiply(checkMatrix(math.identity(n)), gamma_k);
  };

  const push_r_forward = (
    r_i: math.Matrix,
    curr: [[number, number], [math.Matrix, math.Matrix]]
  ): math.Matrix => {
    const [[rho_i, alpha_i], [s_i, y_i]] = curr;
    const beta_i: number = rho_i * math.dot(y_i, r_i);
    const r_i_plus_1 = checkMatrix(
      math.add(r_i, math.multiply(s_i, alpha_i - beta_i))
    );
    return r_i_plus_1;
  };
  ...
  const r_k_minus_m = math.multiply(h_0_k, q_k_minus_m);
...
const lbfgs = (xs: number[], gradfxs: number[], lbfgsInfo: LbfgsParams) => {
  ...
  if (lbfgsInfo.numUnconstrSteps === 0) {
    ...
    const s_km1 = checkMatrix(math.subtract(x_k, x_km1));
    const y_km1 = checkMatrix(math.subtract(grad_fx_k, grad_fx_km1));
    ...
    const descentDirCheck = -1.0 * math.dot(gradPreconditioned, grad_fx_k);
