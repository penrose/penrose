...
const isRectLike = (t: string) =>
  ["Rectangle", "Square", "Image", "Text"].includes(t);
...
const segmentSegmentDist = (s1: VarAD[][], s2: VarAD[][]): VarAD => {
  return min(
    min(pointSegmentDist(s1[0], s2), pointSegmentDist(s1[1], s2)),
    min(pointSegmentDist(s2[0], s1), pointSegmentDist(s2[1], s1))
  );
};

const AABBHausdorffSquaredDistance = (box1: BBox.BBox, box2: BBox.BBox) => {
  ...
  return add(
    squared(max(constOf(0), max(sub(absVal(x0), rx), sub(absVal(x1), rx)))),
    squared(max(constOf(0), max(sub(absVal(y0), ry), sub(absVal(y1), ry))))
  );
};

const AABBNegativeDistance = (box1: BBox.BBox, box2: BBox.BBox) => {
...
const AABBImplementation = (box1: BBox.BBox, box2: BBox.BBox) => {
...
export const exact = (
  ...
  if (!hasExactImpl(t1, t2)) {
    throw Error(
      `exact closest-distance query not supported for ${t1} and ${t2}`
    );
  }
  ...
  if (isRectLike(t1) && isRectLike(t2)) {
    ...
    throw Error(
      `expected an exact closest-distance query implementation for ${t1} and ${t2}, but did not find one`
    );
  }
};
...
export const exactOrLowerBound = (
  [t1, s1]: [string, any],
  [t2, s2]: [string, any]
): VarAD =>
  hasExactImpl(t1, t2)
...
export const exactOrUpperBound = (
  [t1, s1]: [string, any],
  [t2, s2]: [string, any]
): VarAD =>
  hasExactImpl(t1, t2)
