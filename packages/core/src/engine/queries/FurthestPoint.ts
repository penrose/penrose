...
export const hasExactImpl = (t: string) => isRectLike(t) || t === "Circle";

export const lowerBound = ([t, s]: [string, any], dir: VarAD[]): VarAD[] => {
  const box = underboxFromShape(t, s);

  return furthestPointAABB(box, ops.vnormalize(dir));
};

export const exact = ([t, s]: [string, any], dir: VarAD[]): VarAD[] => {
  if (!hasExactImpl(t)) {
  ...
  if (isRectLike(t)) {
    ...
    throw Error(
      `expected an exact furthest point implementation for ${t}, but did not find one`
    );
  }
};

export const upperBound = ([t, s]: [string, any], dir: VarAD[]): VarAD[] => {
...
export const exactOrLowerBound = (
  [t, s]: [string, any],
  dir: VarAD[]
): VarAD[] => (hasExactImpl(t) ? exact([t, s], dir) : lowerBound([t, s], dir));

export const exactOrUpperBound = (
  [t, s]: [string, any],
  dir: VarAD[]
): VarAD[] => (hasExactImpl(t) ? exact([t, s], dir) : upperBound([t, s], dir));
