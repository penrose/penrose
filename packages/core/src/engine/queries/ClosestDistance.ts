...
import { mul, constOf, squared, max, ops, div, add, EPS_DENOM, min, sub, sqrt, absVal, constOfIf } from 'engine/Autodiff';
...
const AABBDistanceSquared = (
  ...
  // https://gamedev.stackexchange.com/a/154040
  const box1 = BBox.inflate(overboxFromShape(t1, s1), constOfIf(padding));
  // const box1 = overboxFromShape(t1, s1);
  const box2 = overboxFromShape(t2, s2);
...
export const exact = (
  ...
  if (t1 === "Line" && t2 === "Line") {
    ...
    return AABBDistanceSquared([t1, s1], [t2, s2], padding);
