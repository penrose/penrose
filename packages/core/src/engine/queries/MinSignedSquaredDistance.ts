...
/**
 * For input shapes A and B, computes min_{a in ∂A} ssd(a, B) where ssd is the signed squared
 * distance computed using B's SDF.
 * 
 * Properties:
 * 
 * minSSD(A, B) > 0 <-> A and B are disjoint. In this case it is equal to the shortest distance
 * between A and B. This value is symmetric.
 * 
 * minSSD(A, B) < 0 <-> A and B intersect. In this case it is equal to the negative Hausdorff
 * distance from A to B for points in the intersection. This value is _not necessarily_ symmetric.
 * 
 * To symmetrize this query, compute min(minSSD(A, B), minSSD(B, A))
 */
...
const AABBImplementation = (
  ...
  const nonNegSqDist = AABBDistanceSquared(box1, box2);
...
export const lowerBound = (
  ...
  [t2, s2]: [string, any],
    ...
    // TODO: double check this is a correct bound in the negative case
