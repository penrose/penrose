...
export const objDict = {
  ...
  // TODO: Revert / factor out with constrDict.disjoint
  disjoint: (
    [t1, s1]: [string, any],
    [t2, s2]: [string, any],
    padding = 0,
...
export const constrDict = {
  ...
  disjoint: (
    ...
    if (IntersectionArea.hasExactImpl(t1, t2)) {
      ...
      // Does it have to be implemented b1 -> b2 or b2 -> b1?

      return ifCond(debug(gt(debug(overlap, "overlap area"), constOf(0.)), "gt res"),
        // Not strong enough? Or is there something wrong with `overlap` or gt?
        // neg(ops.vdist(b1.center, b2.center)),
        neg(debug(ops.vdistsq(b1.center, b2.center), "center distsq")),
        constOf(0.));
 ...
 */
export const closestPt_Box = (p: Pt2, b: BBox.BBox): Pt2 => {
...
/**
 * Return numerically-encoded boolean indicating whether box `a` is contained in box `b`.
 */
export const isContainedInBoxes = (a: BBox.BBox, b: BBox.BBox): VarAD => {
  const fals = constOf(0);
  const tru = constOf(1);

  // b.minX < a.minX && b.maxX > a.maxX && b.minY < a.minY && b.maxY > a.maxY

  const c1 = lt(BBox.minX(b), BBox.minX(a));
  const c2 = gt(BBox.maxX(b), BBox.maxX(a));
  const c3 = lt(BBox.minY(b), BBox.minY(a));
  const c4 = gt(BBox.maxY(b), BBox.maxY(a));

  return ifCond(and(and(and(c1, c2), c3), c4), tru, fals);
};
