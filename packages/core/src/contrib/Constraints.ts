...
const constrDictSimple: { [k: string]: ConstrFunc } = {
  ...
  collinear: constr(
    "collinear",
    [realN("c1"), realN("c2"), realN("c3")],
    (c1: ad.Num[], c2: ad.Num[], c3: ad.Num[]) => {
...
const constrDictGeneral: { [k: string]: ConstrFunc } = {
  ...
  onCanvas: constr(
    "onCanvas",
    [shape("shape"), real("canvasWidth"), real("canvasHeight")],
    (shape: Shape<ad.Num>, canvasWidth: ad.Num, canvasHeight: ad.Num) => {
  ...
  minSize: constr(
    "minSize",
    [shape("shape"), real("limit", 50)],
    (shape: Shape<ad.Num>, limit = 50) => {
  ...
  maxSize: constr(
    "maxSize",
    [shape("shape"), real("limit", 50)],
    (shape: Shape<ad.Num>, limit: ad.Num) => {
  ...
  overlapping: constr(
    "overlapping",
    [shape("s1"), shape("s2"), real("overlap", 0)],
    (s1: Shape<ad.Num>, s2: Shape<ad.Num>, overlap: ad.Num = 0) => {
  ...
  disjoint: constr(
    "disjoint",
    [shape("s1"), shape("s2"), real("overlap", 0)],
    (s1: Shape<ad.Num>, s2: Shape<ad.Num>, padding: ad.Num = 0) =>
  ...
  contains: constr(
    "contains",
    [shape("s1"), shape("s2"), real("overlap", 0)],
    (s1: Shape<ad.Num>, s2: Shape<ad.Num>, padding = 0.0) => {
      const t1 = s1.shapeType,
        t2 = s2.shapeType;
  ...
  atDist: constr(
    "atDist",
    [shape("s1"), shape("s2"), real("distance")],
    (s1: Shape<ad.Num>, s2: Shape<ad.Num>, distance: ad.Num) => {
  ...
  smallerThan: constr(
    "smallerThan",
    [shape("s1"), shape("s2"), real("relativePadding", 0.4)],
    (s1: Shape<ad.Num>, s2: Shape<ad.Num>, relativePadding = 0.4) => {
...
const constrDictSpecific: { [k: string]: ConstrFunc } = {
   ...
   */
  disjointIntervals: constr(
    "disjointIntervals",
    [shape("s1", "Line"), shape("s2", "Line")],
    (s1: Line<ad.Num>, s2: Line<ad.Num>) => {
      return overlap1D(
