...
const graphOfBlockPath = (path: Path): PGraph => {
  const node = {
    data: {
      id: prettyPrintPath(path),
      label: prettyPrintPath(path),
      type: "path",
    },
  };
...
const graphOfFunctionName = (e: Expr): PGraph => {
  // TODO: Fix this if the same fn appears more than once
  if (e.tag === "CompApp" || e.tag === "ObjFn" || e.tag === "ConstrFn") {
    const node = {
      data: {
        id: JSON.stringify(e),
        label: e.name.value,
...
// Written recursively on exprs, just accumulating possible expr errors
export const graphOfBlockExpr = (expr: Expr): PGraph => {
  // Closure for brevity
  const graphOf = graphOfBlockExpr;

  if (isPath(expr)) {
    return graphOfBlockPath(expr);
  } else if (
    expr.tag === "CompApp" ||
    expr.tag === "ObjFn" ||
    expr.tag === "ConstrFn"
  ) {
    const e1 = graphOfFunctionName(expr);
    const e2 = expr.args.map(graphOf);
    return flatGraph([e1].concat(e2));
  } else if (expr.tag === "BinOp") {
    return flatGraph([graphOf(expr.left), graphOf(expr.right)]);
  } else if (expr.tag === "UOp") {
    return graphOf(expr.arg);
  } else if (
    expr.tag === "List" ||
    expr.tag === "Vector" ||
    expr.tag === "Matrix"
  ) {
    return flatGraph(expr.contents.map(graphOf));
  } else if (expr.tag === "ListAccess") {
    return emptyGraph();
  } else if (expr.tag === "GPIDecl") {
    throw Error("Shouldn't exist in live translation");
    // const e1: Graph = graphOfGPIInfo(expr);
    // const e2: Graph[] = expr.properties.map((p) => graphOf(p.value));
    // return flatGraph([e1].concat(e2));
  } else if (expr.tag === "Layering") {
    return flatGraph([graphOf(expr.below), graphOf(expr.above)]);
  } else if (expr.tag === "PluginAccess") {
    return flatGraph([graphOf(expr.contents[1]), graphOf(expr.contents[2])]);
  } else if (expr.tag === "Tuple") {
    return flatGraph([graphOf(expr.contents[0]), graphOf(expr.contents[1])]);
  } else if (expr.tag === "VectorAccess") {
    return graphOf(expr.contents[1]);
  } else if (expr.tag === "MatrixAccess") {
    return flatGraph(expr.contents[1].map(graphOf));
  } else if (
    expr.tag === "Fix" ||
    expr.tag === "Vary" ||
    expr.tag === "StringLit" ||
    ...
    // TODO < Fill in this const
    const node = {
      data: {
        id: JSON.stringify(expr),
        label: "const",
        type: "const",
      },
    };
