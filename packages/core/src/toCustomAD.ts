import {API, ASTPath, BinaryExpression, CallExpression, Collection, FileInfo, Identifier, Node,
    TSTypeReference, UnaryExpression, MemberExpression, ConditionalExpression, NumericLiteral, BooleanLiteral, ArrayExpression, StringLiteral, ArrowFunctionExpression} from 'jscodeshift'
...
module.exports = function(fileInfo: FileInfo, api: API) {
    ...
    const ARRFNEXP2STR = (n: ArrowFunctionExpression) => {
        ...
        else if (n.params.length === 1) params = TOSTRING[n.params[0].type](n.params[0])
        else {
