...
import           Debug.Trace
import           Penrose.Env                      (Arg (..), BaseParser, Parser,
                                                   ParserError (StyleError),
                                                   T (..),
                                                   TypeCtorApp (TypeCtorApp, argCons, constructorInvokerPos, nameCons),
                                                   TypeVar (TypeVar, typeVarName, typeVarPos),
                                                   ValConstructor (tlsvc, tvc),
                                                   Var (..),
                                                   VarEnv (errors, operators, valConstructors, varMap),
                                                   aps, backticks, braces,
                                                   brackets, checkT, colon,
                                                   comma, def, dot,
                                                   doubleQuotedString, eq,
                                                   float, integer, isSubtype,
                                                   isSubtypeArrow, lexeme,
                                                   newline', parens, question,
                                                   quotes, rword, scn, semi',
                                                   symbol, tryChoice, typesEq,
                                                   validChar, var)
...
data Import =
  Import ModulePath (Maybe String)

data ModulePath =
  ModulePath String (Maybe String)
...
data DeclPattern =
  PatternDecl StyT BindingForm
  deriving (Show, Eq, Typeable)

data RelationPattern
  = RelBind BindingForm SelExpr
  | RelPred Predicate
  deriving (Show, Eq, Typeable)
...
toSubPredArg :: PredArg -> C.SubPredArg
toSubPredArg (PE selExpr) = C.PE $ toSubExpr selExpr
toSubPredArg (PP pred)    = C.PP $ toSubPred pred

toSubPred :: Predicate -> C.SubPredicate
toSubPred stypred =
  C.SubPredicate
    { C.predicateName = predicateName stypred
...
mkLocalVarName (LocalVarId (blockNum, substNum)) =
  localKeyword ++ "_block" ++ show blockNum ++ "_subst" ++ show substNum
...
substituteBlockExpr lv subst expr =
  case expr of
    ...
    VectorAccess e1 e2 ->
      VectorAccess
        (substitutePath lv subst e1)
        (substituteBlockExpr lv subst e2)
    MatrixAccess e1 es ->
      MatrixAccess
        (substitutePath lv subst e1)
        (map (substituteBlockExpr lv subst) es)

substituteLine :: LocalVarSubst -> Subst -> Stmt -> Stmt
substituteLine lv subst line =
  case line of
    PathAssign t path expr ->
      PathAssign
        t
        (substitutePath lv subst path)
        (substituteBlockExpr lv subst expr)
...
exprsMatch :: VarEnv -> C.SubExpr -> C.SubExpr -> Bool
-- rule Match-Expr-Var
...
couldMatchRels typeEnv rels stmt = any (flip couldMatchARel stmt) rels
  where
    ...
    couldMatchARel (RelPred styPred) (C.ApplyP subPred) =
      predicateName styPred == C.predicateName subPred
...
find_substs_prog ::
     VarEnv -> C.SubEnv -> C.SubProg -> HeaderBlocks -> [SelEnv] -> [[Subst]]
...
data Translation a =
  Trans
    { trMap    :: M.Map String (FieldDict a)
    , warnings :: [Warning]
    }
  deriving (Show, Eq, Typeable)
...
pathStr (AccessPath p indices) =
  pathStr p ++ "[" ++ intercalate "," (map show indices) ++ "]"
   ...
   in if null (sErrors selEnv) && null bErrs
        then let subst = M.empty -- is this the correct empty?
             -- This is a namespace, not selector, so we substitute local vars with the namespace's name
              in translateBlock (Just name) (block, blockNum) trans (subst, 0) -- skip transSubstsBlock; only one subst
...
evalPluginAccess valMap trans =
  ...
  where
    ...
    evalPluginExpr vmap (VectorAccess e1 e2) =
      VectorAccess e1 (evalPluginExpr vmap e2)
    evalPluginExpr vmap (MatrixAccess e1 es) =
      MatrixAccess e1 (map (evalPluginExpr vmap) es)
...
nameAnonStatements p = map (\(h, b) -> (h, nameAnonBlock b)) p
  where
    ...
    nameAnonStatement (i, b) (AnonAssign e) =
      let stmt = PathAssign Nothing (LocalVar $ anonKeyword ++ "_" ++ show i) e
       in (i + 1, b ++ [stmt])
